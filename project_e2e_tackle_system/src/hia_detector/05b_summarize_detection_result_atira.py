...
atira_cfg = config["ATIRA"]
config = config["hia_detector"]
    
class ResultSummarizerATIRA:
    ...
    def make_figure(self, result: np.ndarray, savename: str, title: str) -> None:
        ...
        fig_w = int(max(10, self.seqlen/10))
    ...
    def _prepare_ground_truth(self, video_file: str) -> np.ndarray:
        ...
        is_target_row = self.df_labels.index == video_file[:-4]
        label = self.df_labels.loc[is_target_row, atira_cfg["label_row"]].values[0]

        gt_labels = np.zeros([2, self.seqlen])

        # Last 2 frames as tackle frame.
        gt_labels[1, -1*atira_cfg["n_last"]:] = 1
        if label == "HR":
            gt_labels[0, -1*atira_cfg["n_last"]:] = 1
        return gt_labels
    ...
    def summarize(self, video_file: str) -> List:
        ...
        if basename not in self.df_labels.index:
            return None, None
        ...
        self.seqlen = len(tackle_frames) # overwrite
...
if __name__ == "__main__":
    ...
    parser.add_argument('--clf', type=str, default="no_clf")
    ...
    result = []
    gts, preds = [], []
    summarizer = ResultSummarizerATIRA(
    ...
    result_text = (
        ...
        f"F1: {f1_score(gt, pred):.04f}\n"
        f"Acc: {accuracy_score(gt, pred):.04f}\n"
        f"Recall: {recall_score(gt, pred):.04f}\n"
        f"Precision: {precision_score(gt, pred):.04f}\n"
